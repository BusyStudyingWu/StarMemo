...
extension NativeTextViewCoordinator {
    ...
    public func textViewDidChangeSelection(_ notification: Notification) {
        ...
        if !tv.isEditable {
            ...
            activeTokenIndices = []
            let paragraphs = tokenRestyleParagraphs(
                in: text,
                tokens: parsed.tokens,
                currentActiveTokenIndices: [],
                previousActiveTokenIndices: previous
            )
            if !previous.isEmpty {
                restyleTextView(tv, paragraphCandidates: paragraphs, tokens: parsed.tokens)
            }
            previousActiveTokenIndices = []
            return
        }

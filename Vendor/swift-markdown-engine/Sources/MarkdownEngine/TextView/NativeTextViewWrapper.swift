...
public struct NativeTextViewWrapper: NSViewRepresentable {
    ...
    public func updateNSView(_ nsView: NSScrollView, context: Context) {
        ...
        // A host update must not replace a still-uncommitted IME composition
        // with the last committed Binding value.
        if textView.hasMarkedText(), context.coordinator.documentId == documentId { return }
        ...
        let themeChanged = context.coordinator.configuration.theme != configuration.theme
        if themeChanged {
            context.coordinator.configuration.theme = configuration.theme
            (textView as? NativeTextView)?.configuration.theme = configuration.theme
        }
        textView.isEditable = isEditable
        textView.isSelectable = isEditable
        ...
        // Retheme the existing editor: keep selection, viewport and undo history.
        let isStyleOnlyChange = (themeChanged || fontChanged) && !isNodeSwitch
            && context.coordinator.lastSyncedText == text
        let preservedSelection = isStyleOnlyChange ? textView.selectedRanges : nil
        let preservedScroll = isStyleOnlyChange ? nsView.contentView.bounds.origin : nil
        let font = NSFont(name: fontName, size: fontSize) ?? NSFont.systemFont(ofSize: fontSize)
        textView.font = font
        ...
        context.coordinator.rebuildTextStorageAndStyle(
            textView,
            from: text,
            invalidateLayout: isNodeSwitch || themeChanged
        )
        ...
        if let preservedScroll {
            nsView.contentView.scroll(to: preservedScroll)

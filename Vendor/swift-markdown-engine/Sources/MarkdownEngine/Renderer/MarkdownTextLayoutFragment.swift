...
final class MarkdownTextLayoutFragment: NSTextLayoutFragment {
    ...
    private func drawTaskCheckboxes(at point: CGPoint, in context: CGContext) {
        ...
        ts.enumerateAttribute(.taskCheckbox, in: range, options: []) { [weak self] value, attrRange, _ in
            ...
            if isChecked {
                configuration.theme.taskCheckboxCheckedFill.setFill()
                checkboxPath.fill()
                ...
                configuration.theme.taskCheckboxCheckmark.setStroke()
                checkPath.stroke()
            } else {
                configuration.theme.taskCheckboxUncheckedFill.setFill()
                checkboxPath.fill()
                configuration.theme.taskCheckboxUncheckedStroke.setStroke()
                checkboxPath.lineWidth = configuration.checkbox.uncheckedStrokeWidth
                checkboxPath.stroke()
            }
        }
    }
